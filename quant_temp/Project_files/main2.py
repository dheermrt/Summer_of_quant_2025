...
def process_data(data):
    ...
    data['roc_ema']=ta.ema(data['roc'],timeperiod=3)
    data['vol_ema']=ta.ema(data['vol'],timeperiod=14)
    data['vol_std']=ta.stdev(data['vol'],timeperiod=14)
...
def strat(data):
    ...
    for i in range(14,len(data)):
        if (position==0):
            if(data.loc[i,'vol']>(data.loc[i,'vol_ema']+data.loc[i,'vol_std']) and data.loc[i,'roc_ema']>0):
                if(data.loc[i,'rsi']<80):
                    ...
                    position=1
            elif(data.loc[i,'vol']>(data.loc[i,'vol_ema']+data.loc[i,'vol_std']) and data.loc[i,'roc_ema']<0):
                data.loc[i,'signals']=-1
            ...
            if(data.loc[i,'roc']<0):
                ...
                days_low=0
            if(data.loc[i,'vol']>(data.loc[i,'vol_ema']+data.loc[i,'vol_std']) and days_low>10):
                data.loc[i,'signals']=-1
            ...
            if(data.loc[i,'vol']>(data.loc[i,'vol_ema']+data.loc[i,'vol_std']) and data.loc[i,'roc_ema']>0):
                ...
                position=1
            elif(data.loc[i,'roc_ema']>0 and days_low<3):
                data.loc[i,'signals']=1
                data.loc[i,'trade_type']='CLOSE'
                position=0
...
def main():
    ...
    csv_file_path = "main2.csv" 
    result_data.to_csv(csv_file_path, index=False)

    bt = BackTester("BTC", signal_data_path="main2.csv", master_file_path="main2.csv", compound_flag=1)
    bt.get_trades(1000)
